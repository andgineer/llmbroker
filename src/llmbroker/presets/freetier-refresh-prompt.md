...
- **A model `src/llmbroker/presets/paid-catalog.toml` lists does not belong in this pool.**
  Pool entries here are named `<provider id>-<model id>`, and a model declared
  from the catalog resolves to a machine-formed name of exactly that shape, so a
  model held by both files collides and a provision refuses it. If the
