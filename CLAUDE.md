...
- **`mission.md`'s "The design this produced" has one job: the load-bearing decisions, for a human who will not read `rules/`.** Its test is predictive, not factual — an item belongs there when a reader who skips the rule files would otherwise mispredict how the library behaves, or would propose a change that fights the design without knowing it. Being true is not enough; most rules are true and still belong only in their own file. Each item is a decision and its *why*, in a few lines, never the mechanism that carries it out (no call counts, no thresholds, no order of operations, no exception lists) — the section is what stays right while the machinery changes underneath. It has no fixed length, but every item added is one the whole section's readers must now pay for, so adding one is a claim that it changes what a reader would expect. This section is the mission's failure mode: check it against the code whenever the corresponding subsystem changes.
