...
- `catalog.py`'s degradation measure counts "distinct `api_key_ref` among **pooled** entries";
  that code and the `architecture.md` sentence it produced become "among **managed** entries".
  Same set, one definition. Its empty-pool case survives the rename rather than going away — a
  registry of only `[[custom]]` entries lands where a `pool = false`-only one does today, and
  `direct=` makes that shape common, so keep the measure silent there.
