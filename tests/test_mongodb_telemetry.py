"""Integration tests for mongodb.Telemetry."""

from datetime import UTC, datetime, timedelta

import pytest

pytest.importorskip("motor.motor_asyncio")

from llmbroker.models import Call, CallStatus, Usage
from llmbroker.mongodb import Telemetry


def _call(call_id="c1", llm_name="p1", user_id=None, **kw):
    return Call(
        id=call_id,
        llm_name=llm_name,
        operation=None,
        trace_id=None,
        status=CallStatus.OK,
        user_id=user_id,
        **kw,
    )


@pytest.fixture(autouse=True)
async def clean(mongo_db):
    yield
    await mongo_db["llmbroker_calls"].delete_many({})


async def test_record_and_calls(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call("c1"))
    calls = await tel.calls(limit=10)
    assert len(calls) == 1
    assert calls[0].id == "c1"


async def test_calls_respects_limit(mongo_db):
    tel = Telemetry(mongo_db)
    for i in range(5):
        await tel.record(_call(f"c{i}"))
    calls = await tel.calls(limit=3)
    assert len(calls) == 3


async def test_calls_user_id_scoping(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call("ca", user_id="alice"))
    await tel.record(_call("cb", user_id="bob"))
    alice_calls = await tel.calls(limit=10, user_id="alice")
    assert len(alice_calls) == 1
    assert alice_calls[0].id == "ca"


async def test_calls_roundtrips_usage_with_extra(mongo_db):
    tel = Telemetry(mongo_db)
    usage = Usage(prompt_tokens=10, completion_tokens=20, total_tokens=30, extra={"cached": 5})
    await tel.record(_call(usage=usage))
    calls = await tel.calls(limit=1)
    u = calls[0].usage
    assert u is not None
    assert u.prompt_tokens == 10
    assert u.extra == {"cached": 5}


async def test_calls_none_usage_roundtrips_as_none(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call())
    calls = await tel.calls(limit=1)
    assert calls[0].usage is None


async def test_record_quality_updates_score(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call())
    await tel.record_quality("c1", 0.8)
    calls = await tel.calls(limit=1)
    assert calls[0].quality_score == pytest.approx(0.8)


async def test_record_quality_missing_raises_key_error(mongo_db):
    tel = Telemetry(mongo_db)
    with pytest.raises(KeyError):
        await tel.record_quality("nonexistent", 1.0)


async def test_purge_calls_removes_old_records(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call())
    deleted = await tel.purge_calls(before=datetime.now(UTC) + timedelta(seconds=5))
    assert deleted == 1
    assert await tel.calls(limit=10) == []


async def test_purge_calls_returns_zero_when_nothing_qualifies(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call())
    deleted = await tel.purge_calls(before=datetime.now(UTC) - timedelta(days=1))
    assert deleted == 0


async def test_metrics_counts_calls_per_llm(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call("c1", llm_name="llm1"))
    await tel.record(_call("c2", llm_name="llm1"))
    await tel.record(_call("c3", llm_name="llm2"))
    m = await tel.metrics()
    assert m["llm1"].call_count == 2
    assert m["llm2"].call_count == 1


async def test_metrics_since_filters_past_calls(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call())
    m = await tel.metrics(since=datetime.now(UTC) + timedelta(seconds=5))
    assert m == {}


async def test_metrics_user_id_scoping(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.record(_call("ca", llm_name="llm1", user_id="alice"))
    await tel.record(_call("cb", llm_name="llm1", user_id="bob"))
    m_alice = await tel.metrics(user_id="alice")
    m_bob = await tel.metrics(user_id="bob")
    m_unscoped = await tel.metrics()
    assert m_alice["llm1"].call_count == 1
    assert m_bob["llm1"].call_count == 1
    assert m_unscoped == {}


async def test_aclose_is_noop(mongo_db):
    tel = Telemetry(mongo_db)
    await tel.aclose()
