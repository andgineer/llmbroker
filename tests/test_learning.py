...
from llmbroker.sqlite.telemetry import Telemetry as SqliteTelemetry
...
def _hook(opt: Optimizer, telemetry, pool: LLMPool) -> _LearningHook:
...
async def test_auth_failure_401_drops_llm_and_logs(caplog):
    ...
    hook = _hook(Optimizer(), NoTelemetry(), pool)
...
async def test_rebuild_loads_quality_windows_from_journal(tmp_path):
    telemetry = SqliteTelemetry(tmp_path / "t.db")
    pool = LLMPool()
    await pool.add(_cfg("bad"), "key")
    opt = Optimizer(quality_min_count=10, quality_floor=0.3)
    hook = _hook(opt, telemetry, pool)

    for _ in range(10):
        await telemetry.record_quality("bad", "summarize", 0.0)
...
async def test_rebuild_computes_metrics_cache(tmp_path):
    ...
    await telemetry.record(_call("x", CallStatus.OK))
    await telemetry.record(_call("x", CallStatus.OK))
...
async def test_rebuild_seeds_and_applies_disabled_map(tmp_path):
    ...
    pool = LLMPool()
    await pool.add(_cfg("x"), "key")
    hook = _hook(Optimizer(), telemetry, pool)

    await telemetry.set_disabled("x", True)
    await hook.maybe_rebuild(force=True)

    assert pool.is_disabled("x")
    assert await telemetry.get_disabled("x") is True
...
async def test_rebuild_seeds_missing_names_as_not_disabled(tmp_path):
    ...
    assert await telemetry.get_disabled("fresh") is False
...
async def test_rebuild_re_enable_via_disabled_map_clears_pool_flag(tmp_path):
    ...
    await telemetry.set_disabled("x", False)
    await hook.maybe_rebuild(force=True)
...
async def test_maybe_rebuild_calls_resync_registry(tmp_path):
    ...
    hook = _LearningHook(Optimizer(), telemetry, pool, resync)
    await hook.maybe_rebuild(force=True)
