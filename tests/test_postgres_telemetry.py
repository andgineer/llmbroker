"""Integration tests for postgres.Telemetry."""
...
from llmbroker.postgres import Telemetry
...
async def test_record_and_calls(pg_pool):
    tel = Telemetry(pg_pool)
...
async def test_calls_respects_limit(pg_pool):
    tel = Telemetry(pg_pool)
    for i in range(5):
...
async def test_calls_user_id_scoping(pg_pool):
    tel = Telemetry(pg_pool)
...
async def test_calls_roundtrips_usage_with_extra(pg_pool):
    tel = Telemetry(pg_pool)
...
async def test_calls_none_usage_roundtrips_as_none(pg_pool):
    tel = Telemetry(pg_pool)
...
async def test_record_quality_updates_score(pg_pool):
    tel = Telemetry(pg_pool)
    await tel.record(_call())
...
async def test_record_quality_missing_raises_key_error(pg_pool):
    tel = Telemetry(pg_pool)
    with pytest.raises(KeyError):
        await tel.record_quality("nonexistent", 1.0)


async def test_purge_calls_removes_old_records(pg_pool):
    tel = Telemetry(pg_pool)
    await tel.record(_call())
...
async def test_purge_calls_returns_zero_when_nothing_qualifies(pg_pool):
    tel = Telemetry(pg_pool)
    await tel.record(_call())
...
async def test_metrics_counts_calls_per_llm(pg_pool):
    tel = Telemetry(pg_pool)
...
async def test_metrics_since_filters_past_calls(pg_pool):
    tel = Telemetry(pg_pool)
    await tel.record(_call())
...
async def test_metrics_user_id_scoping(pg_pool):
    tel = Telemetry(pg_pool)
