...
In async code that is `await llms.snapshot()`. The one object also answers a
whole admin screen: the per-model rows and the pool-wide verdict arrive together,
with no second query to fetch them.
...
A key revoked in your secrets backend drops out of the count on the next
reconcile, so the numbers never lag behind your keys. A model you
[disabled yourself](disable.md) still counts its provider: that verdict is on the
model's own row.

### What to alert on {#alerts}

llmbroker logs the same verdict, so alerting needs no polling of `snapshot()`.
Both lines are `ERROR` on the `llmbroker.broker` logger, one per change of state:
the second fires even when the first already has, since losing your last provider
is its own event. Both name the refs that are missing:
...
Recovery logs one `INFO` (`pool recovered: 3 of 3 providers usable`); gaining a
further provider after that is not worth a line.
...
as a context manager (`with Broker(...) as llms`) initializes the pool up front:
