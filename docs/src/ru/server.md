...
`sync` никогда не уменьшает число моделей, которые пул может вызвать. Модель,
которую новый список выбросил, удаляется только если за неё «заплатила» пришедшая
запись — тем же `api_key_ref` либо собственным ключом; иначе она остаётся и
продолжает работать. Возвращаемый `SyncReport` говорит, что именно произошло, на
каждом запуске, включая запуск без изменений. Ненулевой код выхода задачи и её
лог — тот же канал для админа, которым уже пользуется упавшая миграция; кому
нужно переслать это дальше, читает `llms.last_sync_report`.

### Ключи на пользователя: `have_keys` {#have-keys}

При `scope=` ключи принадлежат пользователям, поэтому синхронизации нечего
пробовать среди общих ключей — и она почти ничего не удаляет: остаётся всё, для
чего не доказано наличие замены. Если вы знаете, что ключ для какого-то ref у
этой инсталляции есть, скажите об этом:

```python
llms = llmbroker.AsyncBroker("postgresql://host/db", have_keys=["OPENAI_API_KEY"])
```

Объявленные ref учитываются только там, где синхронизация решает, может ли
пришедшая запись оплатить удаление; `have_keys` никогда не делает модель
вызываемой — пулу по-прежнему нужен настоящий ключ. Это обещание, и цена ошибки
честная в обе стороны: не укажете — список сохранит записи, которые мог бы
почистить; объявите ref и не выдадите ключ — пул деградирует, потому что старые
записи удалятся, а замены останутся неактивными. Больше нигде ничего объявлять не
нужно.
