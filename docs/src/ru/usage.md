...
Стрим, который вы не пустили в явную гонку, закрепляется за моделью, выдавшей первый
кусок текста, и остаётся с ней до конца. Стрим, который вы в гонку пустили, ведёт себя
иначе: все дорожки продолжают работать до целого ответа, потому что «первое слово
быстрее всех» и «ответ быстрее всех» — обычно разные модели:

```python
stream = broker.stream("Вопрос", fastest_of=2, stream_selection_window=1.0)
```

`stream_selection_window` задаётся в секундах, по умолчанию равен `1.0` и имеет смысл
только для стрима больше чем с одной дорожкой. Он говорит, сколько первому выбору пула
даётся на то, чтобы начать, прежде чем вам покажут то, что уже успел выдать сосед. Это
выбор того, что вы **видите** первым, а не того, кто победит. `0` — показывать первый
пришедший текст, чей бы он ни был.

Гонка стрима стоит дороже гонки обычного вызова. Ответ каждой дорожки держится в
памяти, пока гонка не разрешится, а показанный вам текст остаётся предварительным:
если первой закончит другая модель, он будет отозван и заменён целиком — как
`StreamReplacementError`, который ваш код обязан обработать. Склейки не бывает никогда;
как выглядит этот catch — в разделе [Гонка стрима](async.md#racing-a-stream). Обе опции
относятся только к маршрутизируемому пулу: модель, которую вы зовёте по имени через
...
**Брокер его маршрутизирует, но не гарантирует.** Пул разнородный: часть моделей
соблюдает строгую схему при каждой попытке, часть принимает параметр и отвечает в
своей форме. Отличить одно от другого llmbroker не может — для этого надо читать
ответ по вашей схеме, а смысл схемы принадлежит вам. Поэтому продолжайте
проверять то, что пришло.

Эта же проверка и есть решение. Верните её как
[оценку качества](#quality) с `operation=` для этой задачи — и модели, которые
...
[`direct()`](direct.md#params), принимает вместо этого произвольные параметры
...
- **`trace_id` — это идентификатор одного вызова.** Повесить его на несколько
  ...
  нескольких — сохраните его `call_id`, он ровно для этого. Один вызов может
  устроить вам это и сам: гонка `fastest_of` оставляет под одной трассой два
  отвеченных вызова, если закончили обе модели, и ни по одной записи не видно,
  какой ответ отдали вам. Гонку оценивайте через отданный ею результат или хэндл —
  для стрима это [вот этот](async.md#racing-a-stream) — но не по трассе.
