...
больше нет, даёт предупреждение, а не перезапись. Затем — обычный `sync`.
...
пуловые записи, так что изредка оно может совпасть с одной из них. `--merge`
