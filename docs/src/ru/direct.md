...
- `StreamReplacementError` — **пуловый** стрим, пущенный в гонку через `fastest_of`,
  проиграл целому ответу другой дорожки: в `.replacement` — этот ответ, в
  `.streamed_llm_name` — модель, чьи дельты нужно выбросить.
