...
- `StreamStalledError` — **пуловый** стрим молчал дольше заданной вами паузы
  `stall`; в `.llm_name` — модель, в `.elapsed` — сколько вызов успел прожить.
  Связь не рвалась, модель просто перестала выдавать текст.
