...
`[[llms]]`, тот же парсер, та же таблица реестра, но с флагом `custom`. Начните
с шаблона:
...
Так как запись `custom`, обновление пула её не удаляет: `sync` тотально зеркалит
только записи `[[llms]]` (managed-пресет), а строки `[[custom]]` не трогает. И
`sync` зеркалит только конфиг (`base_url` / `model` / `api_key_ref`) — **никогда
значение ключа**; ключ читается из переменной окружения или secrets-бэкенда в
момент вызова, поэтому обновление пресета секрет не трогает.
